...
const configPath = process.argv[2] ?? process.env.MILFORD_CONFIG ?? "milford.config.yaml";
...
const createServer = (factory as Extract<typeof factory, { ok: true }>).value;
...
if (mcp.transport === "stdio") {
  ...
  const http = serveHttp(createServer, { port: mcp.port, host: mcp.host, tokens: mcp.auth.tokens });
  close = http.close;

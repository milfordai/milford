...
const pascal = (id: string) => id.replace(/(^|[^A-Za-z0-9]+)([A-Za-z0-9])/g, (_, __, char: string) => char.toUpperCase()).replace(/[^A-Za-z0-9]/g, "");
...
export function buildOpenApi(flows: FlowInfo[], base: Json = baseSpec()): Json {
  ...
  const idem = generic.parameters.filter((param: Json) => param.in === "header");
  const names = new Set<string>();
  for (const flow of flows) {
    const name = pascal(flow.id);
    if (names.has(name)) throw new Error(`flows "${flow.id}" and another flow map to the same operation name "run${name}"`);
    names.add(name);

    const input = flow.input ?? { type: "object", additionalProperties: true };
    const hasRequired = Array.isArray(flow.input?.required) && flow.input.required.length > 0;
    ...
    spec.paths[`/v1/flows/${flow.id}/run`] = {
      post: {
        ...generic,
        operationId: `run${name}`,
        summary: flow.description ?? `Run the ${flow.id} flow`,
        parameters: idem,

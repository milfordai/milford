...
`fallback` lets a flow prefer a local provider and fall back to a cloud one, or the reverse. When a provider has `decideMany` and fails, the engine retries each decision through the fallback chain.

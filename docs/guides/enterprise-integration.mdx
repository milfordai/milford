...
description: "Call Milford from Spring and other services that consume a message queue."
---
...
class MilfordClient {
    ...
    MilfordClient(@Value("${milford.url}") String url, @Value("${milford.token}") String token) {
...
void onError(TextMessage message) throws JMSException {
    ...
    var result = milford.run("classify-error", Map.of(
...
Run Milford as a central service with two or more replicas, or as a sidecar next to a service that needs low latency. Milford is stateless apart from the idempotency results, circuit breaker state and rate limits, which each replica keeps in memory. See [deployment](/reference/deployment).

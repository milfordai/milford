...
export TYPESAFE_API_KEY=... LOAGE_TOKEN=change-me
...
The flow names a provider `jev`. Point that id at any provider that can `decide`. The config in the example uses Jev with a local OpenAI-compatible server as a fallback. Replace the entry with an `openai`, `anthropic`, `http` or `sagemaker` provider and the flow stays the same.
